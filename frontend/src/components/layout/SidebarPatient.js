...
import { X, Home, Calendar, History, Settings, LogOut } from 'lucide-react';
import NavItem from '../common/NavItem';
import { useNavigate, useLocation } from 'react-router-dom';

const SidebarPatient = ({ isOpen, onClose }) => {
  const navigate = useNavigate();
  const location = useLocation();

  // Extract the active tab from the current pathname
            ...
            <nav className="flex-1 overflow-y-auto p-4">
              <ul className="space-y-2">
                {[
                  ...
                  { icon: Calendar, text: 'Book Appointment', id: 'book-appointment' },
                  ...
                  <NavItem
                    ...
                    onClick={() => {
                      navigate('/patientDashboard/' + item.id);
                    }}
                  />
                ))}
              </ul>
